...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($judul)) {
        ...
        // Komposisi wajib: minimal 3 soal per kategori (total minimal 15)
        $rekap = array_fill_keys(SOAL_KATEGORI_LIST, 0);
        foreach ($pertanyaans as $idx => $teks) {
            if (trim($teks) === '') continue;
            $kat = $kategoris[$idx] ?? 'umum';
            $kat = in_array($kat, SOAL_KATEGORI_LIST, true) ? $kat : 'umum';
            $rekap[$kat]++;
        }
        $masalah = soal_validasi_komposisi($rekap);
        if ($masalah) {
            $swal_type = 'error';
            $message = 'Komposisi soal minimal 3 per kategori (total minimal 15). Periksa: ' . implode(', ', $masalah);
        } else {
            try {
                ...
                foreach ($pertanyaans as $idx => $teks) {
                    ...
                    $pdo->prepare("INSERT INTO pertanyaan (bank_soal_id, teks_pertanyaan, kategori, urutan) VALUES (?, ?, ?, ?)")
                        ->execute([$bankId, $teks, $kategori, $idx + 1]);
...
<body class="flex flex-col md:flex-row min-h-screen">
    ...
    <template id="templateSoal">
        <div class="soal-block bg-gray-50 rounded-3xl p-6 border border-gray-100" data-index="__IDX__">
            ...
            <textarea name="pertanyaan[__IDX__]" rows="2" required
                      ...
                      class="w-full px-4 py-3 rounded-2xl bg-white border border-gray-200 focus:border-orange-500 outline-none font-semibold text-sm resize-none mb-3"></textarea>
    ...
    function tambahSoal() {
        ...
        soalEl.querySelector('.kategori-select').addEventListener('change', updateRekap);
    ...
    document.getElementById('formSoal').addEventListener('submit', function(e) {
        ...
        // Komposisi: minimal 3 soal per kategori (total minimal 15)
        const counts = {};
        Object.keys(KATEGORI_LABEL).forEach(k => counts[k] = 0);
        soals.forEach(soal => {
            const sel = soal.querySelector('.kategori-select');
            if (sel && counts[sel.value] !== undefined) counts[sel.value]++;
        });
        const kurang = Object.keys(KATEGORI_LABEL).filter(k => counts[k] < 3);
        if (kurang.length > 0) {
            e.preventDefault();
            alert('Komposisi soal minimal 3 per kategori (total minimal 15). Periksa: ' + kurang.map(k => KATEGORI_LABEL[k]).join(', '));
        }
    });

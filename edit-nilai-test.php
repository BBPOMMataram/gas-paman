...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Input ulang pasangan (agen, paket) yang sudah ada = update, bukan duplikat
        if ($idUpdate <= 0) {
            $stmtDup = $pdo->prepare("SELECT id FROM hasil_test WHERE user_id = ? AND bank_soal_id = ?");
            $stmtDup->execute([$userId, $bankId]);
            $dupId = $stmtDup->fetchColumn();
            if ($dupId) $idUpdate = (int)$dupId;
        }

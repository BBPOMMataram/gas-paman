...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['minta_revisi']) && $role === 'admin') {
    ...
    try {
        ...
        try {
            ...
            notifikasi_kirim($pdo, (int)$detail['user_id'], 'laporan_revisi',
                'Laporan perlu direvisi',
                'Admin meminta revisi: ' . mb_substr($pesan, 0, 300),
                'edit-catatan?id=' . $id,
                $id
            );
        } catch (Throwable $e) {}
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                ...
                $labelStatus = match($status) {
                    'approved' => 'Disetujui',
                    'revisi'   => 'Perlu Revisi',
                    'draft'    => 'Draft (Belum Terkirim)',
                    default    => 'Menunggu Review',
                };
                ?>
                <div class="flex items-center gap-3 flex-wrap">
                    <?php if ($role === 'agen' && in_array($status, ['draft', 'pending', 'revisi'], true)): ?>
            ...
            <div class="bg-white rounded-[48px] shadow-2xl shadow-red-900/5 border border-gray-100 overflow-hidden">
                ...
                <div class="bg-gray-50/50 p-8 md:p-10 border-t border-gray-100 flex flex-wrap gap-4 justify-center md:justify-end">
                    ...
                    <?php elseif ($role === 'agen' && in_array($status, ['draft', 'pending', 'revisi'], true)): ?>

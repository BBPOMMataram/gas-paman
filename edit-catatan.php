...
// Hanya boleh edit jika draft / pending / revisi (belum di-approve)
if (!in_array($catatan['status_review'], ['draft', 'pending', 'revisi'], true)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Draft = belum terkirim; kirim = pending (menunggu review admin)
        ...
        $vals = [
            ...
            $informasi, $lokasi, $alamat, $no_hp, $statusReview,
        ];
        if ($hasNilaiPre) {
            $sets[] = 'nilai_pre_test = ?';
            $vals[] = $nilai_pre_test;
        ...
        if (!empty($_FILES['bukti_kegiatan']['name'][0])) {
            foreach ($_FILES['bukti_kegiatan']['tmp_name'] as $key => $tmp_name) {
                ...
                // Ekstensi dari MIME terverifikasi, bukan nama file asli (anti shell.php.jpg)
        ...
        // Log hanya jika benar-benar dikirim (bukan draft)
        if (!$isDraft) {
            $aksiLog = ($catatan['status_review'] === 'draft') ? 'buat' : 'edit';
            $ketLog = ($aksiLog === 'buat')
                ? ('Mengirim laporan dari draft: ' . $nama_konsumen)
                : ('Mengedit laporan: ' . $nama_konsumen);
            log_laporan($pdo, $user_id, $aksiLog, $id, $ketLog);
            try {
                require_once __DIR__ . '/core/notifikasi.php';
                $namaAgen = $_SESSION['nama'] ?? 'Agen';
                $judul = ($aksiLog === 'buat') ? ('Laporan baru dari ' . $namaAgen) : ('Laporan diperbarui: ' . $namaAgen);
                notifikasi_kirim_role($pdo, 'admin', 'laporan_baru', $judul,
                    'Konsumen: ' . $nama_konsumen,
                    'detail-catatan?id=' . $id, $id);
            } catch (Throwable $e) {}
            header('Location: detail-catatan?id=' . $id . '&msg=edited');
        } else {
            header('Location: riwayat?status=draft&msg=draft_saved');
        }
        exit;
    } catch (Exception $e) {
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <form method="POST" enctype="multipart/form-data" class="bg-white p-8 md:p-12 rounded-[48px] shadow-2xl shadow-red-900/10 border border-gray-100 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button type="submit" name="simpan_sebagai" value="draft" class="w-full bg-white border-2 border-blue-500 text-blue-700 hover:bg-blue-50 font-black py-5 rounded-[28px] shadow-sm uppercase text-xs tracking-widest">
                        <i class="fas fa-save mr-2"></i> Simpan sebagai Draft
                    </button>
                    <button type="submit" name="simpan_sebagai" value="kirim" class="w-full bg-orange-600 hover:bg-orange-700 text-white font-black py-5 rounded-[28px] shadow-xl uppercase text-xs tracking-widest">
                        <i class="fas fa-paper-plane mr-2"></i> Kirim / Simpan Laporan
                    </button>
                </div>
                <p class="text-center text-[10px] text-gray-400 font-semibold mt-2">Draft = belum terkirim. Kirim = status pending menunggu review admin.</p>
            </form>
        </div>
    </main>

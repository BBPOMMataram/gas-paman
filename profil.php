...
/**
 * Proses gambar tanda tangan agar menyatu dengan sertifikat:
 * - Latar putih/hampir putih dijadikan transparan (untuk foto hasil scan)
 * - Gambar dipotong otomatis mengikuti batas goresan tinta (auto-crop) + padding
 * - Ukuran dinormalkan (tinggi maks 200px, lebar maks 900px) — tidak diupscale
 * - Hasil selalu disimpan ulang sebagai PNG transparan
 */
function proses_gambar_ttd($path) {
    ...
    // 1) Kanvas RGBA transparan, salin piksel: putih -> transparan, tinta -> tetap
    $dst = imagecreatetruecolor($w, $h);
    imagesavealpha($dst, true);
    imagealphablending($dst, false);
    imagefill($dst, 0, 0, imagecolorallocatealpha($dst, 0, 0, 0, 127));

    $minX = $w; $maxX = -1; $minY = $h; $maxY = -1; $adaTinta = false;
    for ($y = 0; $y < $h; $y++) {
        for ($x = 0; $x < $w; $x++) {
            ...
            $g = ($rgb >> 8) & 255;
            $b = $rgb & 255;
            $aAsli = ($rgb >> 24) & 0x7F; // alpha PNG: 0 = opak
            $lum = (int)(0.299 * $r + 0.587 * $g + 0.114 * $b);

            // putih hampir murni -> transparan; tepi abu lembut; di bawah 205 -> tinta opak
            if ($lum > 238) continue;
            if ($lum > 205) {
                $aOut = (int)(($lum - 205) / 33 * 110) + 17;
            } else {
                $aOut = 0;
            }
            $aOut = min(127, max($aOut, $aAsli));
            $warna = imagecolorallocatealpha($dst, min(255, $r + 12), min(255, $g + 12), min(255, $b + 12), $aOut);
            imagesetpixel($dst, $x, $y, $warna);
    ...
    // 2) Auto-crop mengikuti batas tinta + padding 8% (kalau ada tinta)
    if ($adaTinta && $maxX - $minX > 4 && $maxY - $minY > 4) {
        $padX = max(8, (int)(($maxX - $minX) * 0.08));
        $padY = max(8, (int)(($maxY - $minY) * 0.08));
        $cropX = max(0, $minX - $padX);
        $cropY = max(0, $minY - $padY);
        $cropW = min($w - $cropX, ($maxX - $minX) + 2 * $padX);
        $cropH = min($h - $cropY, ($maxY - $minY) + 2 * $padY);
        $cropped = imagecreatetruecolor($cropW, $cropH);
        imagesavealpha($cropped, true);
        imagealphablending($cropped, false);
        imagefill($cropped, 0, 0, imagecolorallocatealpha($cropped, 0, 0, 0, 127));
        imagecopy($cropped, $dst, 0, 0, $cropX, $cropY, $cropW, $cropH);
        imagedestroy($dst);
        $dst = $cropped;
        $w = $cropW; $h = $cropH;
    }

    // 3) Normalisasi ukuran: tinggi maks 200px / lebar maks 900px, proporsional
    $skala = min(1, 200 / max(1, $h), 900 / max(1, $w));
    if ($skala < 1) {
        ...
        imagecopyresampled($resized, $dst, 0, 0, 0, 0, $nw, $nh, $w, $h);
        imagedestroy($dst);
        $dst = $resized;
    }

    // 4) Simpan ulang sebagai PNG transparan
    ...
    try {
        if (isset($_POST['update_profil'])) {
            ...
            if ($kab_kota && $kecamatan && $desa) {
                // Dikirim dari form dropdown alamat (masih ada di form admin/staff).
                $alamat = ntb_format_alamat($kab_kota, $kecamatan, $desa, $alamat_detail);
            } elseif (isset($_POST['alamat'])) {
                $alamat = htmlspecialchars(trim($_POST['alamat']));
            } else {
                // Agen: field alamat sudah dipindah ke halaman registrasi dan
                // gak ada lagi di form profil, jadi pertahankan alamat lama
                // (jangan sampai ke-timpa kosong tiap kali simpan profil).
                $alamat = $user['alamat'] ?? '';
            }
            ...
            if (($latitude === null || $longitude === null) && $kab_kota && $kecamatan && $desa) {
            ...
            // Field khusus agen
            // ID Agen: pakai yang sudah ada; jika kosong, generate otomatis
            if ($isAgen) {
                $agen_id = trim($user['agen_id'] ?? '');
                if ($agen_id === '') {
                    require_once __DIR__ . '/core/agen_id.php';
                    backfill_agen_ids($pdo);
                    $st = $pdo->prepare("SELECT agen_id FROM users WHERE id = ?");
                    $st->execute([(int)$_SESSION['user_id']]);
                    $agen_id = (string)($st->fetchColumn() ?: generate_next_agen_id($pdo));
                }
            } else {
                $agen_id = $user['agen_id'] ?? null;
            }
            // Kolom nik dihapus dari sistem

            if ($nama === '') throw new Exception("Nama tidak boleh kosong.");
            // agen_id sudah dijamin terisi di atas
            ...
            $update = $pdo->prepare("UPDATE users SET nama = ?, agen_id = ?, jenis_kelamin = ?, usia = ?, alamat = ?, nama_instansi = ?, nomor_hp = ?, pekerjaan = ?, foto_profil = ?, latitude = COALESCE(?, latitude), longitude = COALESCE(?, longitude) WHERE id = ?");
            $update->execute([
                $nama,
                $agen_id,
                $jenis_kelamin,
        ...
        if (isset($_POST['update_ttd']) && $user['role'] === 'kabalai') {
            ...
            // Rapikan gambar: putih -> transparan, auto-crop ke goresan tinta, normalisasi ukuran.
            // Hasil akhir selalu PNG transparan agar tanda tangan menyatu dengan sertifikat.
            $ttdPath = "uploads/" . $new_ttd_name;
            proses_gambar_ttd($ttdPath);
            $ext = strtolower(pathinfo($new_ttd_name, PATHINFO_EXTENSION));
            if ($ext !== 'png') {
                $pngName = preg_replace('/\.(jpe?g|webp)$/i', '.png', $new_ttd_name);
                if (file_exists($ttdPath) && rename($ttdPath, "uploads/" . $pngName)) {
                    $new_ttd_name = $pngName;
                }
            }
                    ...
                    <div class="bg-white p-10 rounded-[45px] shadow-sm border border-gray-50">
                        ...
                        <form action="" method="POST" enctype="multipart/form-data" class="space-y-8" id="profil-form">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                ...
                                <div class="md:col-span-2">
                                    <label class="text-[10px] font-black text-red-800 uppercase tracking-[0.2em] block mb-2.5 ml-1">ID Agen (otomatis)</label>
                                    <input type="text" name="agen_id" value="<?= htmlspecialchars($user['agen_id'] ?? '') ?>" readonly class="soft-input border-red-100 bg-red-50/30 cursor-not-allowed opacity-90">
                                    <p class="text-[10px] text-gray-400 font-medium mt-1 ml-1">ID Agen digenerate otomatis saat registrasi (berdasarkan urutan daftar).</p>
                                </div>

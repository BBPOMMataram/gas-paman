...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Status: draft (belum terkirim) atau pending (sudah dikirim, menunggu review)
        $isDraft = isset($_POST['simpan_sebagai']) && $_POST['simpan_sebagai'] === 'draft';
        ...
        // Bukti wajib hanya saat KIRIM (pending). Draft boleh disimpan tanpa bukti,
        // sesuai janji UI: "simpan sebagai Draft jika belum siap mengirim".
        if (!$isDraft && empty($_FILES['bukti_kegiatan']['name'][0])) {
        ...
        $statusReview = $isDraft ? 'draft' : 'pending';
        ...
        $stmt->execute([$user_id, $tanggal, $nama_konsumen, $usia, $jenis_kelamin, $pekerjaan, $informasi, $lokasi, $alamat, $no_hp, $nilai_pre_test, $nilai_post_test, $lampiran_hasil_test, $latitude, $longitude, $statusReview]);
        ...
        if (!empty($_FILES['bukti_kegiatan']['name'][0])) {
            foreach ($_FILES['bukti_kegiatan']['tmp_name'] as $key => $tmp_name) {
                ...
                // KEAMANAN: tolak semua tipe selain foto/video (PHP, HTML, SVG, dll)
                if (!$is_image && !$is_video) continue;

                // Ekstensi diambil dari MIME yang terverifikasi, BUKAN dari nama file
                // asli — mencegah upload file berbahaya (mis. shell.php.jpg).
                $ext = $is_image ? ($mime === 'image/png' ? 'png' : 'jpg') : 'mp4';
        ...
        // Catat log aktivitas HANYA jika laporan benar-benar dikirim (bukan draft)
        if (!$isDraft) {
            $logOk = false;
            try {
            ...
            // Notif ke admin & staff
            try {
                require_once __DIR__ . '/core/notifikasi.php';
                $namaAgen = $_SESSION['nama'] ?? 'Agen';
                notifikasi_kirim_role($pdo, 'admin', 'laporan_baru',
                    'Laporan baru dari ' . $namaAgen,
                    'Agen mengirim laporan untuk konsumen: ' . $nama_konsumen . '. Silakan review.',
                    'detail-catatan?id=' . (int)$catatan_id,
                    (int)$catatan_id
                );
                notifikasi_kirim_role($pdo, 'staff', 'laporan_baru',
                    'Laporan baru dari ' . $namaAgen,
                    'Agen mengirim laporan untuk konsumen: ' . $nama_konsumen . '.',
                    'detail-catatan?id=' . (int)$catatan_id,
                    (int)$catatan_id
                );
            } catch (Throwable $e) { /* ignore */ }
            ...
            $message = "<div class='bg-blue-100 text-blue-700 p-4 rounded-2xl mb-6 font-bold text-center border border-blue-200 animate-pulse'>Draft berhasil disimpan! Anda bisa melengkapi dan mengirimnya nanti dari menu Riwayat → Draft.</div>";
        }
    } catch (Exception $e) {
...
<head>
    ...
    <title>Tambah Laporan | BBPOM Diary</title>
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data" class="bg-white p-8 md:p-12 rounded-[48px] shadow-2xl shadow-red-900/10 border border-gray-100 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button type="submit" name="simpan_sebagai" value="draft" class="w-full bg-white border-2 border-blue-500 text-blue-700 hover:bg-blue-50 font-black py-5 rounded-[32px] shadow-sm transition-all transform active:scale-[0.98] text-base">
                        <i class="fas fa-save mr-2"></i> Simpan sebagai Draft
                    </button>
                    <button type="submit" name="simpan_sebagai" value="kirim" class="w-full bg-orange-600 hover:bg-orange-700 text-white font-black py-5 rounded-[32px] shadow-xl shadow-orange-900/10 transition-all transform active:scale-[0.98] hover:-translate-y-1 text-base">
                        <i class="fas fa-paper-plane mr-2"></i> Kirim Laporan Sekarang
                    </button>
                </div>
                <p class="text-center text-[10px] text-gray-400 font-semibold mt-2">Draft = belum terkirim ke admin. Anda bisa melengkapi data nanti.</p>
            </form>
        </div>
    </main>
...
<script>
    ...
    // Validasi Submit & SweetAlert — bukti wajib hanya saat KIRIM, tidak wajib untuk DRAFT
    uploadForm.addEventListener('submit', function(e) {
        const submitter = e.submitter;
        const isDraft = submitter && submitter.name === 'simpan_sebagai' && submitter.value === 'draft';
        if (!isDraft && fileInput.files.length === 0) {
            e.preventDefault(); 
            ...
            Swal.fire({
                ...
                text: 'Anda wajib melampirkan minimal 1 foto/video sebagai bukti laporan. Atau simpan sebagai Draft jika belum siap mengirim.',

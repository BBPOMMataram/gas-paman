...
$role = $_SESSION['role'] ?? '';
$isAgen = ($role === 'agen');
$isPengawas = in_array($role, ['admin', 'staff', 'kabalai'], true);

// Hanya agen (milik sendiri) atau admin/staff/kabalai yang boleh lihat detail
if (!$isAgen && !$isPengawas) {
...
$backUrl  = $isAgen ? 'hasil-test' : 'hasil-test-admin';
if (!$hasilId) { header("Location: $backUrl"); exit; }

// Ambil data hasil test
// - Agen: hanya milik sendiri
// - Admin / staff / kabalai: boleh lihat hasil agen mana saja (+ nama agen)
if ($isAgen) {
    ...
    $stmtHasil = $pdo->prepare("
        ...
        WHERE ht.id = ?
    ");
    $stmtHasil->execute([$hasilId]);
}
$hasil = $stmtHasil->fetch();
if (!$hasil) { header("Location: $backUrl"); exit; }

// Ambil soal yang dikerjakan: set tersimpan (hasil_test_soal) bila ada;
// fallback ke semua soal paket untuk hasil legacy / input manual.
require_once 'core/soal_parser.php';
$soalList = [];
$pakaiSet = false;
try {
    hasil_test_soal_ensure_table($pdo);
    $stmtSet = $pdo->prepare("SELECT pertanyaan_id FROM hasil_test_soal WHERE hasil_test_id = ? ORDER BY urutan");
    $stmtSet->execute([$hasilId]);
    $setPids = $stmtSet->fetchAll(PDO::FETCH_COLUMN);
    if ($setPids) {
        $ph = implode(',', array_fill(0, count($setPids), '?'));
        $stmtSoal = $pdo->prepare("
            ...
            WHERE p.bank_soal_id = ? AND p.id IN ($ph)
        ");
        $stmtSoal->execute(array_merge([$hasilId, $hasil['bank_soal_id']], $setPids));
        $byId = [];
        foreach ($stmtSoal->fetchAll() as $s) $byId[$s['pid']] = $s;
        foreach ($setPids as $pid) {
            if (isset($byId[$pid])) $soalList[] = $byId[$pid];
        }
        $pakaiSet = true;
    }
} catch (Throwable $e) {
    error_log('detail-hasil-test hasil_test_soal: ' . $e->getMessage());
}

if (!$pakaiSet) {
    // Fallback: semua pertanyaan dalam bank soal ini (hasil legacy / input manual)
    $stmtSoal = $pdo->prepare("
        SELECT
            p.id as pid,
            p.teks_pertanyaan,
...
<body class="flex flex-col md:flex-row min-h-screen">
    ...
    <main class="flex-1 p-6 md:p-10 overflow-y-auto">
        <div class="max-w-3xl mx-auto">
            ...
            <header class="mb-8">
                <a href="<?= htmlspecialchars($backUrl) ?>" class="inline-flex items-center text-red-800 font-black text-xs uppercase tracking-widest hover:translate-x-1 transition-transform">
            ...
            <div class="<?= $isPreTest ? 'bg-orange-600' : 'bg-red-800' ?> text-white rounded-3xl p-6 mb-8">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <?php if ($isPengawas && !empty($hasil['nama_agen'])): ?>
                        <p class="text-xs <?= $isPreTest ? 'text-orange-100' : 'text-red-100' ?> mt-1 font-semibold">
                            <i class="fas fa-user mr-1"></i>
                            <?= htmlspecialchars($hasil['nama_agen']) ?>
                            <?php if (!empty($hasil['kode_agen'])): ?>
                                <span class="opacity-70">(<?= htmlspecialchars($hasil['kode_agen']) ?>)</span>
                            <?php endif; ?>
                        </p>
                        <?php endif; ?>
            ...
            <div class="bg-white rounded-3xl border <?= $isBenar ? 'border-green-200' : 'border-red-200' ?> shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-5">
                    ...
                    <?php foreach ($opsiList as $opsi):
                        ...
                        $labelJawab = $isPengawas ? 'Jawaban Agen' : 'Jawaban Kamu';
                        if ($isOpsiBenar && $isDipilih) {
                            // Dipilih & benar
                            $rowCls  = 'border-green-400 bg-green-50';
                            $dotCls  = 'bg-green-500';
                            $textCls = 'text-green-800';
                            $badge   = '<span class="text-[8px] font-black px-2 py-0.5 bg-green-200 text-green-700 rounded-lg uppercase tracking-widest">' . $labelJawab . ' ✓</span>';
                            ...
                            $badge   = '<span class="text-[8px] font-black px-2 py-0.5 bg-red-200 text-red-700 rounded-lg uppercase tracking-widest">' . $labelJawab . ' ✗</span>';
                        } else {
                            // Tidak dipilih & salah
            ...
            <div class="mt-8 text-center">
                <a href="<?= htmlspecialchars($backUrl) ?>" class="inline-flex items-center gap-2 bg-red-800 hover:bg-black text-white text-xs font-black uppercase tracking-widest px-8 py-4 rounded-2xl transition-all">

    ...
    if ($isLocked) {
        ...
        if (!empty($email) && !empty($password) && !empty($user_captcha)) {
            ...
            if ($user_captcha != ($_SESSION['captcha_code'] ?? null)) {
                ...
                // Tab "Admin / Staff" = semua role non-agen (admin, kabalai, staff).
                // Tab "Agen Lapangan" = khusus role agen.
                $roleCocok = $user && (
                    ($selected_role === 'admin' && in_array($user['role'], ['admin', 'kabalai', 'staff'], true)) ||
                    ($selected_role === 'agen'  && $user['role'] === 'agen')
                );

                if ($user && ($user['status'] ?? 'aktif') !== 'aktif') {
                    $swal_type = 'error';
                    $swal_msg = "Akun Anda saat ini dinonaktifkan. Silakan hubungi admin BBPOM untuk informasi lebih lanjut.";
...
<body class="fx-auth min-h-screen flex items-center justify-center p-4">

    <div class="fx-auth-panel fx-beam relative flex w-full max-w-[1000px] bg-white rounded-[50px] shadow-[0_20px_60px_-15px_rgba(153,27,27,0.2)] overflow-hidden min-h-[680px] border border-orange-50">

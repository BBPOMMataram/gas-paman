...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($judul)) {
        ...
        try {
            ...
            // Hapus dulu riwayat jawaban lama milik pertanyaan paket ini.
            // FK detail_jawaban.pertanyaan_id tidak pakai ON DELETE CASCADE,
            // jadi tanpa langkah ini edit selalu gagal setelah test pernah
            // dikerjakan agen. Ringkasan hasil_test (nilai, benar, total)
            // tetap dipertahankan.
            $stmtPidsLama = $pdo->prepare("SELECT id FROM pertanyaan WHERE bank_soal_id = ?");
            $stmtPidsLama->execute([$id]);
            $pidsLama = $stmtPidsLama->fetchAll(PDO::FETCH_COLUMN);
            if ($pidsLama) {
                $phLama = implode(',', array_fill(0, count($pidsLama), '?'));
                $pdo->prepare("DELETE FROM detail_jawaban WHERE pertanyaan_id IN ($phLama)")->execute($pidsLama);
            }
            ...
            foreach ($pertanyaans as $idx => $teks) {
                ...
                $kategori = $kategoris[$idx] ?? 'umum';
                $kategori = in_array($kategori, SOAL_KATEGORI_LIST, true) ? $kategori : 'umum';
                $pdo->prepare("INSERT INTO pertanyaan (bank_soal_id, teks_pertanyaan, kategori, urutan) VALUES (?, ?, ?, ?)")
                    ->execute([$id, $teks, $kategori, $idx + 1]);
...
<body class="flex flex-col md:flex-row min-h-screen">
            ...
            <div class="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden">
                ...
                <form action="" method="POST" class="p-8 space-y-8" id="formSoal">
                    ...
                    <div>
                        ...
                        <div id="rekapKategori" class="flex flex-wrap gap-2 mb-4"></div>
    ...
    <template id="templateSoal">
        <div class="soal-block bg-gray-50 rounded-3xl p-6 border border-gray-100" data-index="__IDX__">
            ...
            <textarea name="pertanyaan[__IDX__]" rows="2" required
                      ...
                      class="w-full px-4 py-3 rounded-2xl bg-white border border-gray-200 focus:border-orange-500 outline-none font-semibold text-sm resize-none mb-3">__TEKS__</textarea>

            <div class="mb-4">
                <label class="block text-[9px] font-black uppercase tracking-widest text-gray-400 mb-2">Kategori Soal</label>
                <select name="kategori[__IDX__]" class="kategori-select w-full px-4 py-3 rounded-2xl bg-white border border-gray-200 focus:border-orange-500 outline-none font-bold text-sm">
                    <?php foreach (SOAL_KATEGORI_LIST as $katKey): ?>
                    <option value="<?= $katKey ?>"><?= SOAL_KATEGORI_LABEL[$katKey] ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    const KATEGORI_LABEL = <?= json_encode(SOAL_KATEGORI_LABEL) ?>;

    function updateRekap() {
        const counts = {};
        Object.keys(KATEGORI_LABEL).forEach(k => counts[k] = 0);
        document.querySelectorAll('#soalContainer .kategori-select').forEach(sel => {
            if (counts[sel.value] !== undefined) counts[sel.value]++;
        });
        const el = document.getElementById('rekapKategori');
        el.innerHTML = '';
        Object.keys(KATEGORI_LABEL).forEach(k => {
            const ok = counts[k] >= 3;
            const chip = document.createElement('span');
            chip.className = 'px-2.5 py-1 rounded-lg text-[10px] font-black border ' + (ok ? 'bg-green-50 text-green-700 border-green-200' : 'bg-orange-50 text-orange-600 border-orange-200');
            chip.textContent = KATEGORI_LABEL[k] + ' (' + counts[k] + '/min 3)';
            el.appendChild(chip);
        });
    }
    ...
    function tambahSoal(teks = '', opsiData = [], kategori = 'umum') {
        const idx = soalCount;
        ...
        const kategoriSel = soalEl.querySelector('.kategori-select');
        if (kategoriSel) kategoriSel.value = kategori;
        kategoriSel.addEventListener('change', updateRekap);
    ...
    if (existingData.length > 0) {
        existingData.forEach(s => tambahSoal(s.teks, s.opsi, s.kategori || 'umum'));
    } else {
        tambahSoal();
    }
    </script>

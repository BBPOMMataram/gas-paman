...
$stmtBank = $pdo->prepare("SELECT * FROM bank_soal WHERE jenis = 'post_test' AND status = 'aktif' ORDER BY tanggal DESC, id DESC LIMIT 1");
$stmtBank->execute();
$bankSoal = $stmtBank->fetch();

if (!$bankSoal) {
    $_SESSION['flash_message'] = 'Belum ada paket Post-Test yang aktif. Hubungi admin untuk mengaktifkan soal.';
    $_SESSION['flash_type'] = 'info';
    header("Location: dashboard");
    exit;
}

// Cek apakah sudah mengerjakan pre-test (paket pre_test yang aktif, atau pre_test manapun)
$stmtCekPre = $pdo->prepare("
    SELECT ht.id FROM hasil_test ht
    JOIN bank_soal bs ON bs.id = ht.bank_soal_id
    WHERE ht.user_id = ? AND bs.jenis = 'pre_test'
    LIMIT 1
");
$stmtCekPre->execute([$userId]);
if (!$stmtCekPre->fetch()) {
    $_SESSION['flash_message'] = 'Kamu harus menyelesaikan Pre-Test terlebih dahulu sebelum mengerjakan Post-Test.';

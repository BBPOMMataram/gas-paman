...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user) {
        ...
        try {
            ...
            $mail->SMTPSecure = ($smtp['encryption'] === 'ssl') ? PHPMailer::ENCRYPTION_SMTPS : (($smtp['encryption'] === 'tls') ? PHPMailer::ENCRYPTION_STARTTLS : '');
            ...
            // URL reset otomatis mengikuti domain tempat aplikasi diakses
            // (tidak hardcoded ke satu hostname — bisa localhost, IP LAN, atau domain produksi)
            $baseUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https' : 'http')
                     . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost')
                     . rtrim(dirname($_SERVER['SCRIPT_NAME'] ?? '/'), '/\\');
            $url = $baseUrl . "/reset-password-aksi?token=$token";
            $mail->isHTML(true);
...
<body class="fx-auth flex items-center justify-center min-h-screen p-6">

    <div class="w-full max-w-[480px]">
        <div class="fx-auth-panel fx-beam bg-white rounded-[60px] shadow-[0_20px_70px_-15px_rgba(153,27,27,0.12)] p-10 md:p-16 border border-orange-50/50 text-center relative overflow-hidden">

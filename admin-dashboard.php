...
// ========== PERSENTASE UNTUK PROGRESS BAR STATISTIK ==========
$totalAllAgen = (int)$pdo->query("SELECT COUNT(*) FROM users WHERE role = 'agen'")->fetchColumn();
$pctAgen = $totalAllAgen > 0 ? (int)round($totalAgen / $totalAllAgen * 100) : 0;
if ($filterPeriode) {
    $st = $pdo->prepare("SELECT COUNT(*) FROM catatan_harian WHERE status_review = 'approved' AND tanggal BETWEEN ? AND ?");
    $st->execute([$tgl_mulai, $tgl_selesai]);
    $totalApproved = (int)$st->fetchColumn();
} else {
    $totalApproved = (int)$pdo->query("SELECT COUNT(*) FROM catatan_harian WHERE status_review = 'approved'")->fetchColumn();
}
$pctApproved = $totalLaporan > 0 ? (int)round($totalApproved / $totalLaporan * 100) : 0;
$pctRasio    = $totalAgen > 0 ? min(100, (int)round($totalLaporan / $totalAgen * 100)) : 0;
$pctPending  = $totalLaporan > 0 ? (int)round($pendingLaporan / $totalLaporan * 100) : 0;
...
<body class="bg-gray-50 flex flex-col md:flex-row min-h-screen">
    ...
    <main class="flex-1 p-6 md:p-12 overflow-y-auto">
            ...
            <div class="flex items-center gap-5">
                ...
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        <span class="px-2 py-0.5 bg-red-800 text-white text-[8px] font-black uppercase tracking-widest rounded-md inline-flex items-center gap-1.5">
                    ...
                    <h2 class="fx-text-gradient text-3xl font-black tracking-tight">Monitoring Admin</h2>
                    <p class="text-gray-500 text-sm font-medium italic">"Keluarga Sadar Obat dan Makanan Aman"</p>
                </div>
            </div>
            <div class="fx-beam bg-white px-5 py-3 rounded-2xl shadow-sm border border-gray-100 flex items-center space-x-3">
                <span class="fx-beam-border"></span>
                <div class="w-10 h-10 bg-gradient-to-br from-red-800 to-red-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-red-800/30">
        ...
        <form method="GET" data-fx-reveal class="bg-white rounded-[28px] border border-gray-100 shadow-sm p-6 mb-8">
            <h3 class="fx-section-title text-xs font-black text-gray-800 uppercase tracking-[0.2em] mb-6">
                <span class="fx-icon-chip text-xs"><i class="fas fa-filter"></i></span>Filter Periode
            </h3>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <a href="daftar-agen" data-fx-reveal class="stat-card bg-white p-6 rounded-[28px] border border-gray-100 relative overflow-hidden group block">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full bg-red-50 text-red-800">Aktif</span>
                </div>
                <h3 class="fx-stat-num text-5xl font-black mt-5" data-fx-count="<?= $totalAgen ?>"><?= $totalAgen ?></h3>
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] mt-1.5">Total Agen Aktif</p>
                <p class="text-[9px] font-bold text-gray-400 mt-3"><?= $pctAgen ?>% dari total agen terdaftar</p>
                <div class="fx-bar mt-1.5" data-fx-w="<?= $pctAgen ?>%"><i></i></div>
                <p class="text-[10px] text-orange-600 font-bold mt-3 flex items-center gap-1">Lihat daftar agen <i class="fas fa-arrow-right text-[9px] group-hover:translate-x-1 transition-transform"></i></p>
                <i class="fas fa-users absolute -right-3 -bottom-4 text-8xl text-red-800/[0.05] group-hover:text-red-800/[0.08] transition-colors"></i>
            </a>

            <a href="riwayat?status=approved" data-fx-reveal class="stat-card bg-white p-6 rounded-[28px] border border-gray-100 relative overflow-hidden group block">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full bg-orange-50 text-orange-700">Edukasi</span>
                </div>
                <h3 class="fx-stat-num text-5xl font-black mt-5" data-fx-count="<?= $totalMasyarakat ?>"><?= number_format($totalMasyarakat) ?></h3>
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] mt-1.5">Masyarakat Teredukasi</p>
                <p class="text-[9px] font-bold text-gray-400 mt-3"><?= $pctApproved ?>% laporan disetujui</p>
                <div class="fx-bar mt-1.5" data-fx-w="<?= $pctApproved ?>%"><i></i></div>
                <p class="text-[10px] text-orange-600 font-bold mt-3 flex items-center gap-1">Lihat detail laporan <i class="fas fa-arrow-right text-[9px] group-hover:translate-x-1 transition-transform"></i></p>
                <i class="fas fa-graduation-cap absolute -right-3 -bottom-4 text-8xl text-orange-600/[0.05] group-hover:text-orange-600/[0.08] transition-colors"></i>
            </a>
            ...
            <a href="riwayat" data-fx-reveal class="stat-card bg-white p-6 rounded-[28px] border border-gray-100 relative overflow-hidden group block">
                <div class="flex items-center justify-between">
                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-red-700 to-red-500 text-white flex items-center justify-center text-lg shadow-lg shadow-red-700/30 group-hover:scale-110 transition-transform">
                        <i class="fas fa-file-alt"></i>
                    </div>
                    <span class="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full bg-gray-100 text-gray-600">Total</span>
                </div>
                <h3 class="fx-stat-num text-5xl font-black mt-5" data-fx-count="<?= $totalLaporan ?>"><?= $totalLaporan ?></h3>
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] mt-1.5">Laporan Masuk</p>
                <p class="text-[9px] font-bold text-gray-400 mt-3"><?= $pctRasio ?>% rasio laporan per agen</p>
                <div class="fx-bar mt-1.5" data-fx-w="<?= $pctRasio ?>%"><i></i></div>
                <p class="text-[10px] text-orange-600 font-bold mt-3 flex items-center gap-1">Semua laporan <i class="fas fa-arrow-right text-[9px] group-hover:translate-x-1 transition-transform"></i></p>
                <i class="fas fa-file-alt absolute -right-3 -bottom-4 text-8xl text-red-700/[0.05] group-hover:text-red-700/[0.08] transition-colors"></i>
            </a>

            <a href="riwayat?status=pending" data-fx-reveal class="stat-card fx-beam bg-white p-6 rounded-[28px] border border-gray-100 relative overflow-hidden group block">
                <span class="fx-beam-border"></span>
                <div class="flex items-center justify-between">
                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-orange-600 to-red-600 text-white flex items-center justify-center text-lg shadow-lg shadow-orange-600/30 group-hover:scale-110 transition-transform">
                        <i class="fas fa-clock"></i>
                    </div>
                    <span class="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full bg-orange-600 text-white inline-flex items-center gap-1.5">
                        <span class="relative flex h-1.5 w-1.5">
                ...
                <h3 class="fx-stat-num text-5xl font-black mt-5" data-fx-count="<?= $pendingLaporan ?>"><?= $pendingLaporan ?></h3>
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] mt-1.5">Menunggu Review</p>
                <p class="text-[9px] font-bold text-gray-400 mt-3"><?= $pctPending ?>% dari total laporan</p>
                <div class="fx-bar mt-1.5" data-fx-w="<?= $pctPending ?>%"><i></i></div>
                <p class="text-[10px] text-orange-600 font-bold mt-3 flex items-center gap-1">Review sekarang <i class="fas fa-arrow-right text-[9px] group-hover:translate-x-1 transition-transform"></i></p>
                <i class="fas fa-clock absolute -right-3 -bottom-4 text-8xl text-orange-600/[0.05] group-hover:text-orange-600/[0.08] transition-colors"></i>
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
            <div class="bg-white p-7 rounded-[32px] border border-gray-100 shadow-sm">
                <p class="fx-section-title text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] mb-4"><span class="fx-icon-chip text-xs"><i class="fas fa-chart-line"></i></span>Rata-rata Nilai Agen</p>
            ...
            <div class="bg-white p-7 rounded-[32px] border border-gray-100 shadow-sm">
                <p class="fx-section-title text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] mb-4"><span class="fx-icon-chip text-xs"><i class="fas fa-users"></i></span>Rata-rata Nilai Masyarakat</p>
                <div class="flex items-end gap-8">
                    <div>
                        <p class="text-[9px] font-bold text-orange-600 uppercase">Pre-Test</p>
            ...
            <div class="bg-white p-7 rounded-[32px] border border-gray-100 shadow-sm">
                <p class="fx-section-title text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] mb-4"><span class="fx-icon-chip text-xs"><i class="fas fa-file-signature"></i></span>Sertifikat Post-Test</p>
                <div class="flex items-end gap-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
            <div class="bg-white p-6 md:p-8 rounded-[32px] border border-gray-100 shadow-sm">
                <h4 class="fx-section-title font-black text-gray-800 uppercase text-xs tracking-[0.2em] mb-4"><span class="fx-icon-chip"><i class="fas fa-map-marker-alt"></i></span>Sebaran Agen (NTB)</h4>
            ...
            <div class="bg-white p-6 md:p-8 rounded-[32px] border border-gray-100 shadow-sm">
                <h4 class="fx-section-title font-black text-gray-800 uppercase text-xs tracking-[0.2em] mb-4"><span class="fx-icon-chip"><i class="fas fa-map-marker-alt"></i></span>Sebaran Masyarakat Teredukasi</h4>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
            <div class="bg-white rounded-[32px] border border-gray-100 shadow-sm p-6 md:p-8">
                <h4 class="fx-section-title font-black text-gray-800 uppercase text-xs tracking-[0.2em] mb-6"><span class="fx-icon-chip"><i class="fas fa-chart-bar"></i></span>Grafik Sebaran Agen per Kabupaten/Kota</h4>
                <div class="chart-box">
                    <canvas id="chartAgen"></canvas>
                </div>
            </div>
            <div class="bg-white rounded-[32px] border border-gray-100 shadow-sm p-6 md:p-8">
                <h4 class="fx-section-title font-black text-gray-800 uppercase text-xs tracking-[0.2em] mb-6"><span class="fx-icon-chip"><i class="fas fa-chart-bar"></i></span>Grafik Sebaran Masyarakat per Kabupaten/Kota</h4>
                <div class="chart-box">

...
// Pastikan semua agen punya ID Agen sequential berdasarkan created_at
if (is_file(__DIR__ . '/core/agen_id.php')) {
...
$sql = "
    ...
    ORDER BY
        CASE
            WHEN u.agen_id IS NULL OR u.agen_id = '' THEN 999999
            ELSE CAST(SUBSTRING(u.agen_id, LOCATE('-', u.agen_id) + 1) AS UNSIGNED)
        END ASC,
        u.created_at ASC,
        u.id ASC
";
...
<body class="bg-gray-50 flex flex-col md:flex-row min-h-screen">
    ...
    <main class="flex-1 p-4 md:p-12 overflow-x-hidden">
        ...
        <form method="GET" class="bg-white rounded-[28px] border border-gray-100 shadow-sm p-6 mb-8 flex flex-wrap items-end gap-4">
            ...
            <?php endif; ?>
            <div class="export-actions ml-auto">
                ...
                <a href="cetak-daftar-agen<?= ($filterPeriode ? '?mulai='.urlencode($tgl_mulai).'&selesai='.urlencode($tgl_selesai) : '') ?>" target="_blank" rel="noopener"

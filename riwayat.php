...
if ($statusFilter === 'draft') {
    // Mode draft: hanya tampilkan draft milik agen (atau filter agen utk admin)
    $query .= " AND c.status_review = 'draft'";
} elseif (in_array($statusFilter, ['pending', 'approved', 'revisi'], true)) {
    $query .= " AND c.status_review = ?";
    $params[] = $statusFilter;
} else {
    // Default: hanya laporan yang SUDAH TERKIRIM (bukan draft)
    $query .= " AND c.status_review != 'draft'";
}
...
if ($viewAdmin) {
    ...
    $st = $pdo->prepare("SELECT COUNT(*) FROM catatan_harian WHERE $baseWhere AND status_review != 'draft'");
...
<body class="bg-gray-50 flex flex-col md:flex-row min-h-screen">
    ...
    <main class="flex-1 p-4 md:p-12 overflow-y-auto">
        ...
        <form method="GET" class="bg-white rounded-[28px] border border-gray-100 shadow-sm p-6 mb-8 flex flex-wrap gap-4 items-end">
            ...
            <?php if ($role === 'agen'): ?>
            <a href="riwayat?status=draft" class="px-6 py-3 <?= $statusFilter==='draft' ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-700 hover:bg-blue-200' ?> text-[10px] font-black uppercase tracking-widest rounded-xl transition-all">
                <i class="fas fa-file-alt mr-1"></i> Draft
            </a>
            <?php endif; ?>
        </form>

        <?php if ($statusFilter === 'draft' && $role === 'agen'): ?>
        <div class="mb-6 p-4 rounded-2xl bg-blue-50 border border-blue-200 text-blue-800 text-sm font-semibold">
            <i class="fas fa-info-circle mr-2"></i> Menampilkan <strong>Draft</strong> — laporan yang belum dikirim ke admin. Klik Edit untuk melengkapi lalu kirim.
        </div>
        <?php endif; ?>

        <?php if (!empty($_GET['msg']) && $_GET['msg'] === 'draft_saved'): ?>
        <div class="mb-6 p-4 rounded-2xl bg-blue-100 border border-blue-200 text-blue-700 text-sm font-bold text-center">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-orange-50/30 transition-all">
                            ...
                            <td class="px-5 py-5 text-center whitespace-nowrap">
                                ...
                                <?php if ($role === 'agen' && in_array($row['status_review'], ['draft', 'pending', 'revisi'], true)): ?>
